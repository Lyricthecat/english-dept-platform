...
(function (global) {
  ...
  App.Grades = {
    ...
    ensureRanks(ex, classId) {
      ...
      // 缺考 / 无总分的学生不参与排名
      const hasVal = recs.filter(r => !App.isAbsent(r) && App.num(r.total) != null);
      ...
      recs.forEach(r => { if (App.isAbsent(r) || App.num(r.total) == null) r.rank = null; });
      if (need) DB().save('更新排名');
    },

    // 一键重算所有参与班级的班内总分排名（覆盖已有排名，并列同名次，缺考不参与）
    recomputeAllRanks(ex) {
      let classN = 0, stuN = 0;
      ex.classIds.forEach(cid => {
        const recs = (ex.scores || {})[cid] || [];
        const valid = recs.filter(r => !App.isAbsent(r) && App.num(r.total) != null);
        ...
        recs.forEach(r => { if (App.isAbsent(r) || App.num(r.total) == null) r.rank = null; });
        classN++; stuN += sorted.length;
      });
      if (!classN) { App.toast('没有可计算排名的成绩数据', 'err'); return { classN: 0, stuN: 0 }; }
      DB().save('重算班内排名');
      App.toast(`已重算 ${classN} 个班、${stuN} 名学生的班内总分排名`, 'ok');
      this.render();
      return { classN, stuN };
    },
    ...
    renderAnalysis(v) {
      ...
      <div class="card">
        <div class="flex mb12">
          ...
          <div class="spacer"></div>
          <button class="btn btn-ghost btn-sm" data-act="recalcRanks">🧮 一键计算班内排名</button>
      ...
      App.delegate(v, {
        click: {
          backExam: () => { this.view = 'exam:' + ex.id; this.render(); },
          recalcRanks: () => this.recomputeAllRanks(ex),
    ...
    _buildGradeAnalysisPdf(ex) {
      ...
      this._pdfPage(P, ctx => {
        ...
        const rows3 = allIds.map(cid => {
          ...
          }), App.fmtPct(statsMap[cid].total.rate)];
        });
